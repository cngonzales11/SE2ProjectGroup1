    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (didHit)
            {
                ...
                if (rhInfo.collider.tag == "enemy")
                {
                    Character enemy = rhInfo.collider.GetComponent<Character>();

                    if (enemy)
                    {
                        if (battleSystem.getInstance().hasCurrentMove())
                        {
                            Debug.Log(enemy.maxHp);
                            battleSystem.getInstance().setTarget(enemy);
                        }
                    }
                }
            }
